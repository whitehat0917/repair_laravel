...
class PostRepository
{
    ...
    public function getAll($nbrPages, $parameters)
    {
        foreach ($parameters as $key=>$value){
            if ($value != ""){
                $parameters[$key] = "posts.".$value;
            }
        }
        // print_r($parameters);
        // die();
        return $this->model->with ('ingoing')
            ->leftJoin('users as user', 'user.id', '=', 'posts.user_id')
            ->select('posts.*','user.email','user.name')
    ...
    {
        return $this->model->select('*')->where('user_id', '=', auth()->id())->get();
    }
}
